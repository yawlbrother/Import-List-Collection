...
- Surface assets carry the two virtual-texture stacks every Colossal surface has (`write_surface(..., vt=(w, h))`): stack 1 = BaseColor, Normal, MaskMap, ControlMask, stack 2 = Emissive, eight 16-byte slots each, unused slots zero, then a 16-byte id. Layout taken byte for byte from the KISS and vanilla prop surfaces (version u16, template u8, three zero bytes, nullable block marker 00/01, u32 stack count, per stack u32 width, u32 height, 8 slots; asset references are tagged ff). Without the block every other map streams fine but the control mask never reaches the normal render path, so colour masks showed only while the vehicle was highlighted. Streaming works in 512 px tiles: a surface whose textures are smaller than that on either side (the 256 x 128 CS1 LOD atlas) is written without the block, because with it the game aborts at start-up ("All sizes need to be bigger than the tileSize!").
