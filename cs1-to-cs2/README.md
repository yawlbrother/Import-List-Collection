...
- Door indicator lights: purposes 69/70 (`BoardingLightLeft/Right`) are lit while the train boards on that side. The CS1 mesh has no lamp geometry for them, so the builder adds a 12 × 6 cm quad above every CS1 door position, pointing at a small painted patch in free atlas space, and gives its vertices index 5 or 6 by the sign of x (left = -x). The simplified LOD1 drops them.
