...
- Line colour: the door leaves take the transport line's colour, like vanilla buses. A `ColorProperties` component on the LOD0 render prefab (one variation, channel 0 bound to the external source `Parent` = the vehicle's route, hue/saturation/value jitter 0) plus a ControlMask whose red channel marks the door-leaf texels (found by geometry: side-facing body triangles next to each CS1 door position, `door_leaf_triangles`). The shader multiplies the base colour by the channel colour, so the masked base texels are lightened to about 165 first, and the variation's own channel colour is the livery's dark grey again for cars that are not on a line and for the static props. Door windows (lit texels) stay unmasked. `preview_line.png` shows the mask in a sample blue.
