...
- Lights: the emissive texture's RGB is the lamp colour and its alpha is a *layer id*; `EmissiveProperties` multi-light mappings bind a layer to a purpose (23 = interior lights, 3 = headlamps, 6 = red rear lamps in the reference) with a `color` for the active state and `colorOff` otherwise. Surfaces need the `_EMISSIVE_PROCEDURAL` keyword. The builder paints windows (CS1 illumination 20-40) on layer 255 and headlamps (illumination >200) on layer 25 with red `colorOff`.
